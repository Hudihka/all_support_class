...
class PinterestLayout: UICollectionViewLayout{
  ...
  private var numberOfColumns: Int {
    return portretOrientation ? 2 : 3       //количество столбцов
  }
  ...
  private var contentWidth: CGFloat { //ширина контента
    ...
    //MARK: ВНИМАНИЕ!!! возможно
    //придется пересчитать значения value
    let insets = collectionView.contentInset
    let value = portretOrientation ? wDdevice : hDdevice
