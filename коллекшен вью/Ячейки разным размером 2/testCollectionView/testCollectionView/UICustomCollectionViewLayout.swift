...
//  
//
//  Created by Hudihka on 08/01/2020.
//
...
public class UICustomCollectionViewLayout: UICollectionViewLayout {
    ...
    override public func prepare() {
        if cache.isEmpty {
            ...
            if self.showHeader {
                headerHeight = 88
                let a = UICollectionViewLayoutAttributes(forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter,
                                                         with: IndexPath(item: 1, section: 0))
                a.frame = CGRect(x: cellPadding,
                                 y: contentHeight + cellPadding,
