...
class Notifications: NSObject, UNUserNotificationCenterDelegate {
    ...
    func getNotificationSettings() {
        notificationCenter.getNotificationSettings { (settings) in
            ...
            DispatchQueue.main.async {              //этот кусок кода нужен для НЕ ЛОКАЛЬНЫХ пуш уведомлений
                UIApplication.shared.registerForRemoteNotifications()   //регестрируем девайс для нотификаций
            }
        }
    }
    ...
    func scheduleNotification(notifaicationType: String) { //собственно запуск пуш уведомления
        ...
        let userAction = "User Action"         //это тип пуш уведомлений, например пуши о необходимости оплатить заказ
                    //ЕСЛИ пуш с сервера содержит "categoru" : "User Action" то на уведомлениях С серверв будут кнопки что ниже
