...
//        sceneView.debugOptions = [ARSCNDebugOptions.showFeaturePoints]
    ...
    @objc func placeVirtualObject(tapGesture: UITapGestureRecognizer) {
        ...
        // здесь мы удаляем все обьектты со сцены
        //это надо для того, чтто бы дым не дублировался
    ...
    func createVirtualObject(hitResult: ARHitTestResult) {
        ...
        //здесь мы создаем дым над трубой

        //в начале берем файл дыма из проекта
        if let particleSystem = SCNParticleSystem(named: "Smoke.scnp", inDirectory: nil),
            //затем из счены берем нод под именем SmokeNode
            let smokeNode = virtualObject.childNode(withName: "SmokeNode", recursively: true) {


            //ну говорим что нодом на сцене являеттся каким файлом
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // For now it is possible to use .vertical orientation BTW
