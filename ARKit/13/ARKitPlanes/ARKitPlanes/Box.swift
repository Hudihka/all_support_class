...
class Box: SCNNode {
    ...
    init(atPosition position: SCNVector3) {
        ...
        //битовая маска применяется здесь

        //какая маска физика у куба

        self.physicsBody?.categoryBitMask = BitMaskCategory.box

        //от обьекта с какой бит маской будет происходитть оттскок(на подобие мяча и сттенки)

        self.physicsBody?.collisionBitMask = BitMaskCategory.box | BitMaskCategory.plane

        //от обьекта с какой бит маской будет происходитть контактт
        //на подобие как обьекта в воде
