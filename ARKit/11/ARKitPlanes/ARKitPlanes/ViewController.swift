...
class ViewController: UIViewController {
    ...
    @objc func placeBox(tapGesture: UITapGestureRecognizer) {
        ...
        //в начале мы получаем локацию тапа жеста
        let sceneView = tapGesture.view as! ARSCNView
        let location = tapGesture.location(in: sceneView)

        //далее по локации создается вектор и по этому вектору в пространстве рисуется куб
    ...
    func createBox(hitResult: ARHitTestResult) {
       //0,05 это половина грани куба
        //остальные величины взяты по 0 что бы куб лег ровно на поверхность
