...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //////////////////// новое
        //этто фишки дебага котторые позволяют собственно дебажитть
        //здесь это добавление тточек сенсоров и оси кординатт
    ...
    func custom(){
        sessionn.pause()
        //мы можем перебрать все обьекты и ищем нужный по имени
        sceneView.scene.rootNode.enumerateChildNodes { (node, _) in
            if node.name == "blba"{
                node.removeFromParentNode()//удаляем нужный
            }
        }

        //теперрь создаем новый куб но так что бы у него была новая позиция оттноситтельно камеры
        let configuration = ARWorldTrackingConfiguration()
        //создаем сессию с новыми параметрами, при учете что удаляемм старый якорь и оси кординат
        sessionn.run(configuration, options: [.removeExistingAnchors, .resetTracking])
    }
    ...
    private func createBox(in scene: SCNScene) {
        ...
        boxNode.name = "__NAME"  //можно задать кастомное имя
