...
class SaveImg: NSObject {
    ...
    func updateUI(imageURL: URL?,
        ...
        if let image = managerDirectory.loadImageFromDiskWith(name: keySave) {
            ...
            //если мы хотим обойтись без подов то закоменти SaveImageClass.dowloadNuke
            //а то что меньше раскоменти
            
            //        } else if keySave.suffix(3) == "svg"{
            //            SaveImageClass.dowloadSVG(imageURL: imageURL, keySave: keySave, imageView: imageView) { (error) in
            //                completion?(error)
            //            }
        } else {
        ...
        Nuke.loadImage(with: imageURL,
                       ...
                       progress: nil) { (loadingImage, error) in
                        ...
                        if let error = error {
                            ...
                            DispatchQueue.global(qos: .userInitiated).async {
                                self.managerDirectory.saveImage(image: image, key: keySave)
                            }
                            completion?(nil)
                        }
        }
